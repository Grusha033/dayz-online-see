...
HELP_BINDS = [
    ("SECTION", "SERVER LIST & CONTROL"),
    ("Select / Query Server", "Enter"),
    ("Navigate Servers", "Up / Down (k / j)"),
    ("Add Server (Finder)", "/"),
    ("Manual Refresh", "r"),
    ("Toggle Auto-Refresh (10s)", "a"),
    ("Remove Server", "Del / d"),
    ("Quit Application", "q"),
    ("SECTION", "CHART NAVIGATION"),
    ("Previous / Next Day", "Left / Right (h / l)"),
    ("Scroll Chart Timeline", "Scroll / < / > (, / .)"),
    ("Move Timeline Cursor", "[ / ] (x / ji)"),
    ("Inspect Sample Point", "Mouse Left Click"),
    ("SECTION", "OTHER"),
    ("Close This Menu", "F1 / Esc"),
]
...
def draw_help_screen(stdscr, max_y, max_x, C_BORDER, C_WHITE, C_ACTIVE, C_HINT, C_TEXT):
    w = min(68, max_x - 4)
    h = min(22, max_y - 4)
    x = (max_x - w) // 2
    y = (max_y - h) // 2

    draw_round_box(stdscr, y, x, h, w, C_ACTIVE)
    
    title = " Key Bindings & Help "
    try:
        stdscr.addstr(y, x + (w - len(title)) // 2, title, C_WHITE)
    except curses.error:
        pass
    ...
    for item in HELP_BINDS:
        ...
        if kind == "SECTION":
            sec_text = f"── {val} "
            sec_fill = sec_text + ("─" * max(0, inner_w - len(sec_text)))
            try:
                stdscr.addstr(cur_row, col_x, sec_fill[:inner_w], C_BORDER)
            except curses.error:
                pass
            cur_row += 1
            continue
        ...
        dots_count = inner_w - len(desc) - len(btn_str) - 2
        if dots_count > 0:
            line_str = desc + " " + ("·" * dots_count) + " " + btn_str
        else:
            line_str = f"{desc} {btn_str}"

        try:
            stdscr.addstr(cur_row, col_x, desc, C_WHITE)
            if dots_count > 0:
                stdscr.addstr(cur_row, col_x + len(desc) + 1, "·" * dots_count, C_TEXT)
            stdscr.addstr(cur_row, col_x + inner_w - len(btn_str), btn_str, C_HINT)
        except curses.error:
            pass

        cur_row += 1

    close_hint = " Press F1 or Esc to return "
    try:
        stdscr.addstr(y + h - 1, x + (w - len(close_hint)) // 2, close_hint, C_ACTIVE)
    except curses.error:
        pass
...
def main(stdscr):
    ...
    while True:
        ...
        if state["show_help"]:
            draw_help_screen(stdscr, max_y, max_x, C_BORDER, C_WHITE, C_ACTIVE, C_HINT, C_TEXT)
            stdscr.refresh()
        else:
            ...
            f1_hint = "[F1] help"
            if recent_w > len(f1_hint) + 4:
                try:
                    stdscr.addstr(1, recent_w - len(f1_hint) - 2, f" {f1_hint} ", C_HINT)
                except curses.error:
                    pass
            ...
            if state["is_loading"]:
                ...
                total_points = len(day_records)
                max_scroll = max(0, total_points - plot_w)
                state["chart_scroll_offset"] = max(0, min(state["chart_scroll_offset"], max_scroll))

                if max_scroll > 0:
                    end_idx = total_points - state["chart_scroll_offset"]
                    start_idx = max(0, end_idx - plot_w)
                    plot_records = day_records[start_idx:end_idx]
                else:
                    start_idx = 0
                ...
                left_overflow = "« " if (start_idx > 0) else "  "
                right_overflow = " »" if (state["chart_scroll_offset"] > 0) else "  "
                
                scroll_info = f" ({len(plot_records)}/{total_points})" if max_scroll > 0 else ""
                hdr_date = f"{nav_arrow_l}{left_overflow}{cur_date} ({date_tag}){right_overflow}{nav_arrow_r}{scroll_info}"
        ...
        if state["show_help"]:
            if code in (curses.KEY_F1, 27) or ch in ("q", "Q", "й", "Й"):
            ...
            try:
                ...
                if bstate & (curses.BUTTON1_CLICKED | curses.BUTTON1_PRESSED | curses.BUTTON1_DOUBLE_CLICKED):
                    ...
                    if hit is not None:
                        ...
                        state["chart_cursor_idx"] = -1
                elif bstate & (curses.BUTTON4_PRESSED if hasattr(curses, "BUTTON4_PRESSED") else 0x10000):
                    if state["chart_scroll_offset"] < max_scroll:
                        state["chart_scroll_offset"] += 3
                        state["chart_cursor_idx"] = -1
                elif bstate & (curses.BUTTON5_PRESSED if hasattr(curses, "BUTTON5_PRESSED") else 0x200000):
                    if state["chart_scroll_offset"] > 0:
                        state["chart_scroll_offset"] -= 3
                        state["chart_cursor_idx"] = -1
            except curses.error:
                pass
            ...
            if ch in ("q", "Q", "й", "Й"):
                ...
                if plot_records:
                    if state["chart_cursor_idx"] == -1:
                        ...
                        state["chart_cursor_idx"] -= 1
                    elif state["chart_cursor_idx"] == 0 and state["chart_scroll_offset"] < max_scroll:
                        state["chart_scroll_offset"] += 1
            elif ch in ("]", "ъ", "Ъ", "ї", "Ї"):
                if plot_records:
                    if state["chart_cursor_idx"] == -1:
                        ...
                        state["chart_cursor_idx"] += 1
                    elif state["chart_cursor_idx"] == len(plot_records) - 1 and state["chart_scroll_offset"] > 0:
                        state["chart_scroll_offset"] -= 1
